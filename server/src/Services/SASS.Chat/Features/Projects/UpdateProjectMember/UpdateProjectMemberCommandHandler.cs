...
internal sealed class UpdateProjectMemberCommandHandler(
    ...
    : IRequestHandler<UpdateProjectMemberCommand, IdResult>
{
    public async Task<IdResult> Handle(UpdateProjectMemberCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!isLeader)
        {
            throw new UnauthorizedAccessException("You do not have permission to update project members.");
        }
