...
public class DocxContentDecoder(IServiceProvider serviceProvider) : IContentDecoder
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var p in body?.Descendants<Paragraph>() ?? []) content.AppendLine(p.InnerText);
        ...
        return Task.FromResult(paragraphs.Select((text, index) => new Chunk(null, index, text)).ToList()
            .AsEnumerable());
    }
}