...
using SASS.SharedKernel.Results;

namespace SASS.Chassis.Endpoints;

/// <summary>
/// Todo: To check later when applying pagination
/// This will add pagination into headers so user can navigate previous or next page
/// </summary>
internal sealed class PaginationHeaderFilter : IEndpointFilter
{
    ...
    private static void AddLinkHeader(HttpContext context, PaginationMetadata metadata)
    {
        ...
        {
            links.Add(BuildLink(basePath, queryParams, metadata.PageIndex + 1, metadata.PageSize, "next"));
        }
